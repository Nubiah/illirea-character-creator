...
function calculHP() {
  ...
  const HPCon =
    parseInt(CharacterData.level) *
    parseInt(CharacterData.stats.CONSTITUTION.value);
...
//On check si Voie des mages est check, et on change sa valeur (true / false) dans l'objet CharacterData
document
  .getElementById("voieMageCheck")
  .addEventListener("change", function () {
    CharacterData.mage = this.checked;
  });

function calculMP() {
  const CharacterVoies = Object.values(CharacterData.voies);
  const voieMagie = CharacterVoies.filter((v) => v.type === "Magie");
  const voieDesMages = CharacterData.mage ? CharacterData.voiePeuple.rang : 0;
  ...
  CharacterData.maxMP = CharacterData.stats.VOLONTE.value + MP + voieDesMages;
...
function chargerStatPeuple(peuple) {
  ...
  const bonus = raceChoisie.statsChoice
    ? raceChoisie.statsChoice.split(", ")
    : "Aucun"; //Condition ? vrai : faux
  const select = document.getElementById("bonusPeuple"); //On récupère la zone
  select.innerHTML =
    '<select name="" id="bonusPeuple" class="form-select"></select>';
...
const profils = [
  ...
  },
  {
    ...
    id: "ensorceleur",
    mainStat: "INT,PER",
    voieInitialesID: "eau, feu, foudre, terre",
    isMagique: true,
  },
  {
    nom: "Mentat",
    id: "mentat",
    mainStat: "INT,WIS",
    voieInitialesID: "télékinésie, pensée, chaman, soigneur",
    isMagique: true,
  },
...
document.getElementById("profil").addEventListener("change", function () {
  ...
  const voiesProfil = profils
    .find((p) => p.id === CharacterData.profil)
    .voieInitialesID.split(",")
    .map((v) => v.trim());
  ...
  voiesSelect1.forEach((select) => {
    select.innerHTML =
      '<option value="" disabled selected>Voie initiale</option>';
  });
  voiesSelect2.forEach((select) => {
    select.innerHTML =
      '<option value="" disabled selected>Voie secondaire</option>';
  });
...
document
  .getElementById("voiePeupleRange")
  .addEventListener("input", function () {
    document.getElementById("voiePeupleRangeValue").textContent =
      `R${this.value}`;
...
function updateStats() {
  ...
  document.getElementById("dexterite").innerHTML =
    CharacterData.stats.DEXTERITE.value;
  document.getElementById("constitution").innerHTML =
    CharacterData.stats.CONSTITUTION.value;
  document.getElementById("intelligence").innerHTML =
    CharacterData.stats.INTELLIGENCE.value;
  document.getElementById("will").innerHTML = CharacterData.stats.VOLONTE.value;
  document.getElementById("charisme").innerHTML =
    CharacterData.stats.CHARISME.value;
}
...
const voies = [
  ...
  { nom: "Voie de la survie", id: "survie", type: "Finesse" },
  {
    ...
    id: "combat_à_deux_armes",
    type: "Finesse",
  },
  {
    nom: "Voie du danseur arcanique",
    id: "danseur_arcanique",
    type: "Finesse",
  },
  ...
  {
    nom: "Voie de l'enchantement d'arme",
    id: "enchantement_d_arme",
    type: "Magie",
  },
